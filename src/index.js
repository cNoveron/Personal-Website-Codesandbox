...
import textureUrl from "./assets/233.jpeg"
import "./styles.css"
...
function Diamonds() {
  ...
  const diamonds = useMemo(
    () =>
      new Array(100).fill().map((_, i) => ({
        position: [i < 5 ? 0 : 5 * viewport.width * (0.5 - Math.random()), 0 * (1 - Math.random()), i < 30 ? -50 : -Math.random() * 100],
        factor: 1.2 * Math.random(),
        direction: Math.random() < 0.5 ? -1 : 1,
        rotation: [Math.sin(Math.random()) * Math.PI, Math.sin(Math.random()) * 40 * Math.PI, Math.cos(Math.random()) * Math.PI]
      })),
    [viewport]
  )
  ...
  useFrame(() => {
    ...
    diamonds.forEach((data, i) => {
      ...
      dummy.rotation.set(rotation[0] + 2 * t * factor, rotation[1] + 2 * t * factor, 2 * rotation[2] + t * factor)
  ...
  return (
    <>
      ...
      <div
        ...
        <a style={{ flex: 1 }} href="https://drive.google.com/file/d/1YkyWbqfO7UzELW4JhLncNPVHC9cu4M5q/view?usp=sharing">
          CV
        </a>
        <a style={{ flex: 1 }} href="https://linkedin.com/in/carlos-noveron">
          LinkedIn
        </a>
        <a style={{ flex: 1 }} href="https://github.com/cNoveron">
          GitHub
        </a>
        <a style={{ flex: 1 }} href="https://drive.google.com/file/d/1L5-2kxxbzZ9j9lz4LJn4y5ghCcKDM0P_/view?usp=sharing">
          NFT Wallet Demo
        </a>
      </div>
    </>
